...
- **Responsive `@media` rule scoped to our modal**
  - The narrow-viewport (`max-width: 600px`) block targeted the bare `.modal` class — every Obsidian modal, not just ours — and used `!important` four times to override Obsidian's default modal width. The selector is now `.modal:has(.word-count-modal)`, which scopes the rule to the plugin's own modal and supplies enough specificity to drop all four `!important` declarations. Trades four `!important` rescan warnings for one `:has()` warning (net -3) and matches the `:has()` pattern already used elsewhere in `styles.css`.

### Internal
- **Release pipeline with verifiable provenance**
  - Added `.github/workflows/release.yml` to build, lint, attest, and create a draft release on every plain SemVer tag (`1.6.6`) or pre-release SemVer tag (`1.6.6-rc1`). Each release asset (`main.js`, `manifest.json`, `styles.css`) now carries a GitHub artifact attestation generated by `actions/attest-build-provenance@v2`. Users (and Obsidian's community-site scanner) can verify provenance with `gh attestation verify main.js --repo banisterious/obsidian-custom-selected-word-count`.
  - Added `.nvmrc` pinning the Node version (22.20.0) that CI and local dev share.
  - Rewrote `docs/developer/release.md` to document the CI-driven flow, the trial-run procedure with `-rc1` tags, the artifact verification command, and the recovery path for a failed CI run.
- **Narrowed caught error types and frontmatter input handling**
  - Surfaced by the new CI lint gate: ten `@typescript-eslint/no-unsafe-*` findings that the catalog rescan does not check but `npm run lint` does. Added a module-level `errorMessage(error: unknown): string` helper and routed the seven `catch` blocks that read `error.message` through it (`instanceof Error` narrowing, falling back to `String(error)` for non-Error throws). Annotated `cache.frontmatter['cswc-disable']` as `unknown`, used a `(item): item is string` type predicate on the array-filter branch, and cast `loadData()`'s return through `Partial<WordCountPluginSettings> | null` in `loadSettings` so the merged settings object is well-typed. No user-visible behavior change; lint now exits clean.
