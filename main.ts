...
import { DEFAULT_SETTINGS, DEFAULT_WORD_REGEX, type WordCountPluginSettings } from './src/settings/types';
import { type CountResult, type WordCountHistoryEntry } from './src/types';
import { debugLog, errorMessage } from './src/utils/debug';
import { getDisabledExclusionsFromFrontmatter, stripFrontmatter } from './src/processing/frontmatter';
import { processTextWithOverrides } from './src/processing/overrides';
import { processCodeBlocks, processInlineCode } from './src/processing/code';
import { processObsidianComments, processHtmlComments } from './src/processing/comments';
import { processLinks } from './src/processing/links';
import { processHeadings } from './src/processing/headings';
import { processWordsAndPhrases } from './src/processing/words-and-phrases';

// Re-export the moved declarations so consumers (including the
// characterization test suite at tests/) keep working with imports
// against `'./main'`. Phase 5 splits the source tree under `src/`;
// these re-exports preserve the public surface during and after that
// split.
